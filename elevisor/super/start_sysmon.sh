...
export JAVA_OPTS="-Dsysmon_home=$SYSMON_HOME -Dsysmon_group=$SYSMON_GROUP_NAME -Dsysmon_name=$SYSMON_NAME -Delevisor_server_ip=$SERVER_IP -Dsysmon_port=$SYSMON_PORT"
...
then
    if [ "$1" = "-d" ] || [ "$2" = "-d" ] 
    then
     $JAVA_HOME/bin/java -DELEVISOR_SYSMON -Xms64m -Xmx512m $JAVA_OPTS -jar $SYSMON_HOME/lib/launcher.jar  com.elevisor.sysmon.agent.SysMonMain $1 $2 
    else
     nohup $JAVA_HOME/bin/java -DELEVISOR_SYSMON -Xms64m -Xmx512m  $JAVA_OPTS -jar $SYSMON_HOME/lib/launcher.jar com.elevisor.sysmon.agent.SysMonMain >> $SYSMON_HOME/logs/sysmon_stdout_$(date +%Y%m%d).log &
     echo " Starting Agent... "
    fi
else
